 ...
 */
export function getBlueprint(name, extendWith) {
    ...
    if (!blueprint) {
        ...
        if (blueprint) {
            ...
            throw new Error(`Trying to retrieve unknown blueprint: ${name}`);
        }
    }
 ...
 */
export function buildEntity(node, blueprint) {
 ...
 */
export function createChild(parent, blueprint) {
    if (typeof (blueprint) === 'string') {
        blueprint = getBlueprint(blueprint);
    }
 ...
 */
export function createChildAtPosition(parent, blueprint, spawnPosition) {
 ...
 */
export function mixinBlueprint(owner, defaults, blueprintSection) {
